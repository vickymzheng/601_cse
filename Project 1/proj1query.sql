...
from CLINICAL_FACT
join DISEASE
on DISEASE.DS_ID = CLNICAL_FACT.DS_ID
WHERE DISEASE.TYPE = 'leukemia';
...
JOIN(SELECT PB_ID
     ...
     JOIN GENE_FACT
     ON GENE_FACT.U_ID = PROBE.U_ID
     WHERE GENE_FACT.GO_ID = '12502')SUB3
ON MICROARRAY_FACT.PB_ID = SUB3.PB_ID);
...
JOIN(SELECT PB_ID
     ...
     JOIN GENE_FACT
     ON GENE_FACT.U_ID = PROBE.U_ID
     WHERE GENE_FACT.GO_ID = '7154')SUB3
ON MICROARRAY_FACT.PB_ID = SUB3.PB_ID);
...
JOIN(SELECT DISTINCT S_ID
     ...
     JOIN(SELECT P_ID
          ...
          JOIN DISEASE
          ON DISEASE.DS_ID = CLINICAL_FACT.DS_ID
          WHERE DISEASE.NAME ='Colon tumor')SUB1
     ON CLINICAL_FACT.P_ID = SUB1.P_ID
     WHERE S_ID <> 'null')SUB2
ON MICROARRAY_FACT.S_ID = SUB2.S_ID
JOIN(SELECT PB_ID
...
JOIN(SELECT DISTINCT S_ID
     ...
     JOIN(SELECT P_ID
          ...
          JOIN DISEASE
          ON DISEASE.DS_ID = CLINICAL_FACT.DS_ID
          WHERE DISEASE.NAME ='Breast tumor')SUB1
     ON CLINICAL_FACT.P_ID = SUB1.P_ID
     WHERE S_ID <> 'null')SUB2
ON MICROARRAY_FACT.S_ID = SUB2.S_ID
JOIN(SELECT PB_ID
